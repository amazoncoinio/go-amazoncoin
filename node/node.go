...
func (n *Node) OpenDatabase(name string, cache, handles int) (amzdb.Database, error) {
	if n.config.DataDir == "" {
		return amzdb.NewMemDatabase()
	}
	return amzdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
