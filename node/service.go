...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (amzdb.Database, error) {
	if ctx.config.DataDir == "" {
		return amzdb.NewMemDatabase()
	}
	db, err := amzdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
