...
import (
	...
	"github.com/amazoncoinio/go-amazoncoin/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of geta is "geta". If no
...
func (c *Config) NodeName() string {
	...
	if name == "geta" || name == "geta-testnet" {
		name = "Geth"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "geta" instances.
...
func (c *Config) resolvePath(path string) string {
	...
	// by geta 1.4 are used if they exist.
	if c.name() == "geta" && isOldGethResource[path] {
		oldpath := ""
		if c.Name == "geta" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	default:
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-amazoncoin-keystore")
		ephemeral = keydir
	}
	if err != nil {
		return nil, "", err
