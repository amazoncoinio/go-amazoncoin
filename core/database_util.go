...
var (
	...
	configPrefix   = []byte("amazoncoin-config-") // config prefix for the db
...
func WriteCanonicalHash(db amzdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db amzdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db amzdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db amzdb.Putter, hash common.Hash) error {
...
func WriteHeader(db amzdb.Putter, header *types.Header) error {
...
func WriteBody(db amzdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db amzdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db amzdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db amzdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db amzdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db amzdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db amzdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db amzdb.Database) amzdb.Database {
	return amzdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db amzdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db amzdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db amzdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
