...
// This file is part of go-amazoncoin.
//
// go-amazoncoin is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// go-amazoncoin is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with go-amazoncoin. If not, see <http://www.gnu.org/licenses/>.

// Package utils contains internal helper functions for go-amazoncoin commands.
package utils
...
import (
	...
	"github.com/amazoncoinio/go-amazoncoin/accounts/keystore"
	...
	"github.com/amazoncoinio/go-amazoncoin/core/state"
	"github.com/amazoncoinio/go-amazoncoin/core/vm"
	"github.com/amazoncoinio/go-amazoncoin/crypto"
	...
	"github.com/amazoncoinio/go-amazoncoin/amzstats"
	"github.com/amazoncoinio/go-amazoncoin/les"
	"github.com/amazoncoinio/go-amazoncoin/log"
	"github.com/amazoncoinio/go-amazoncoin/metrics"
	"github.com/amazoncoinio/go-amazoncoin/node"
	"github.com/amazoncoinio/go-amazoncoin/p2p"
	"github.com/amazoncoinio/go-amazoncoin/p2p/discover"
	"github.com/amazoncoinio/go-amazoncoin/p2p/discv5"
	"github.com/amazoncoinio/go-amazoncoin/p2p/nat"
	"github.com/amazoncoinio/go-amazoncoin/p2p/netutil"
	"github.com/amazoncoinio/go-amazoncoin/params"
	whisper "github.com/amazoncoinio/go-amazoncoin/whisper/whisperv5"
...
var (
	...
	AmazoncoinbaseFlag = cli.StringFlag{
		Name:  "amazoncoinbase",
...
// setAmazoncoinbase retrieves the amazoncoinbase either from the directly specified
// command line flags or from the keystore if CLI indexed.
func setAmazoncoinbase(ctx *cli.Context, ks *keystore.KeyStore, cfg *eth.Config) {
	if ctx.GlobalIsSet(AmazoncoinbaseFlag.Name) {
		account, err := MakeAddress(ks, ctx.GlobalString(AmazoncoinbaseFlag.Name))
		if err != nil {
			Fatalf("Option %q: %v", AmazoncoinbaseFlag.Name, err)
		}
		cfg.Amazoncoinbase = account.Address
		return
	}
	accounts := ks.Accounts()
	if (cfg.Amazoncoinbase == common.Address{}) {
		if len(accounts) > 0 {
			cfg.Amazoncoinbase = accounts[0].Address
		} else {
			log.Warn("No amazoncoinbase set and no accounts found as default")
		}
	}
}
...
func SetNodeConfig(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.GlobalBool(DevModeFlag.Name):
		cfg.DataDir = filepath.Join(os.TempDir(), "amazoncoin_dev_mode")
	case ctx.GlobalBool(TestnetFlag.Name):
...
// RegisterEthService adds an Amazoncoin client to the stack.
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the Amazoncoin service: %v", err)
	}
}
...
// RegisterEthStatsService configures the Amazoncoin Stats daemon and adds it to
// th egiven node.
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both eth and les services
		var ethServ *eth.AmazonCoin
		ctx.Service(&ethServ)

		var lesServ *les.LightAmazonCoin
		ctx.Service(&lesServ)

		return ethstats.New(url, ethServ, lesServ)
	}); err != nil {
		Fatalf("Failed to register the Amazoncoin Stats service: %v", err)
	}
}
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node) amzdb.Database {
	var (
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *core.BlockChain, chainDb amzdb.Database) {
	var err error
...
// new format.
//
// e.g. geta account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// geta --keystore /tmp/mykeystore --lightkdf account new
//
