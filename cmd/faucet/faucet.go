...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Amazoncoin protocol")
	...
	logFlag = flag.Int("loglevel", 3, "Log level to use for Amazoncoin and the faucet")
)
...
type request struct {
	...
	Account  common.Address     `json:"account"`  // Amazoncoin address being funded
	Time     time.Time          `json:"time"`     // Timestamp when te request was accepted
	Tx       *types.Transaction `json:"tx"`       // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Amazoncoin light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Amazoncoin protocol stack
	client *ethclient.Client   // Client connection to the Amazoncoin chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Amazoncoin light client protocol
...
// close terminates the Amazoncoin connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Iterate over all the files and look for Amazoncoin addresses
		...
		if address == (common.Address{}) {
			websocket.JSON.Send(conn, map[string]string{"error": "No Amazoncoin address found to fund"})
			continue
		}
