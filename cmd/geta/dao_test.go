...
func testDAOForkBlockNewChain(t *testing.T, test int, genesis string, expectBlock *big.Int, expectVote bool) {
	...
	if genesis != "" {
		...
		geta := runGeth(t, append(args, []string{"--exec", "2+2", "console"}...)...)
		geta.WaitExit()
	}
	// Retrieve the DAO config flag from the database
	path := filepath.Join(datadir, "geta", "chaindata")
	db, err := amzdb.NewLDBDatabase(path, 0, 0)
	if err != nil {
		t.Fatalf("test %d: failed to open test database: %v", test, err)
	}
	defer db.Close()

	genesisHash := common.HexToHash("0x8b419632ffc3c5a5c3b1990bba0837e612ef584a53b1b864e016fb6894cf6653")
	if genesis != "" {
