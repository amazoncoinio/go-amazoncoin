// Copyright 2017 The go-amazoncoin Authors
...
// nodeDockerfile is the Dockerfile required to run an Amazoncoin node.
var nodeDockerfile = `
FROM amazoncoin/client-go:latest
...
{{end}}
RUN \
  echo 'geta init /genesis.json' > geta.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.amazoncoin/keystore/ && cp /signer.json /root/.amazoncoin/keystore/' >> geta.sh && \{{end}}
	echo $'geta --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .BootV4}}--bootnodesv4 {{.BootV4}}{{end}} {{if .BootV5}}--bootnodesv5 {{.BootV5}}{{end}} {{if .Amazoncoinbase}}--amazoncoinbase {{.Amazoncoinbase}} --mine{{end}}{{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> geta.sh

ENTRYPOINT ["/bin/sh", "geta.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Amazoncoin node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 geta --exec admin.nodeInfo.id attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.amazoncoin"],
		portFull:   infos.portmap[infos.envvars["FULL_PORT"]],
		portLight:  infos.portmap[infos.envvars["LIGHT_PORT"]],
		peersTotal: totalPeers,
		peersLight: lightPeers,
		ethstats:   infos.envvars["STATS_NAME"],
		amazoncoinbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasPrice:   gasPrice,
