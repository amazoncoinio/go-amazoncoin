...
func (p *protips) print(network string) {
	...
	fullinit, lightinit := "", ""
	if p.genesis != "" {
		fullinit = fmt.Sprintf("geta --datadir=$HOME/.%s init %s.json && ", network, network)
		lightinit = fmt.Sprintf("geta --datadir=$HOME/.%s --light init %s.json && ", network, network)
	}
	...
	tips = append(tips, fmt.Sprintf("%sgeta --networkid=%d --datadir=$HOME/.%s --cache=1024%s%s", fullinit, p.network, network, statsflag, bootflagFull))

	tasks = append(tasks, "Run a full node with recent data only")
	tips = append(tips, fmt.Sprintf("%sgeta --networkid=%d --datadir=$HOME/.%s --cache=512 --fast%s%s", fullinit, p.network, network, statsflag, bootflagFull))

	tasks = append(tasks, "Run a light node with on demand retrievals")
	tips = append(tips, fmt.Sprintf("%sgeta --networkid=%d --datadir=$HOME/.%s --light%s%s", lightinit, p.network, network, statsflag, bootflagLight))

	tasks = append(tasks, "Run an embedded node with constrained memory")
	tips = append(tips, fmt.Sprintf("%sgeta --networkid=%d --datadir=$HOME/.%s --cache=32 --light%s%s", lightinit, p.network, network, statsflag, bootflagLight))
