...
func (w *wizard) deployNode(boot bool) {
	...
	if !boot {
		if w.conf.genesis.Config.Ethash != nil {
			// Ethash based miners only need an amazoncoinbase to mine against
			fmt.Println()
			if infos.amazoncoinbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.amazoncoinbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.amazoncoinbase)
				infos.amazoncoinbase = w.readDefaultAddress(common.HexToAddress(infos.amazoncoinbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootFull, w.conf.bootLight, infos); err != nil {
		log.Error("Failed to deploy Amazoncoin node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
