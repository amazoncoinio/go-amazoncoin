...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/amazoncoinio                                                            && \
	(cd $GOPATH/src/github.com/amazoncoinio && git clone --depth=1 https://github.com/amazoncoinio/go-amazoncoin) && \
  go build -v github.com/amazoncoinio/go-amazoncoin/cmd/faucet                                              && \
