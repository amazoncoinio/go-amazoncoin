...
import (
	...
	"github.com/amazoncoinio/go-amazoncoin/logger"
	"github.com/amazoncoinio/go-amazoncoin/p2p"
	"github.com/amazoncoinio/go-amazoncoin/p2p/nat"
	"github.com/amazoncoinio/go-amazoncoin/whisper"
)
...
func main() {
	...
	// Create an Amazoncoin peer to communicate through
	server := p2p.Server{
		PrivateKey: key,
		MaxPeers:   10,
		Name:       name,
	...
	fmt.Println("Starting Amazoncoin peer...")
	if err := server.Start(); err != nil {
		fmt.Printf("Failed to start Amazoncoin peer: %v.\n", err)
		os.Exit(1)
	}
