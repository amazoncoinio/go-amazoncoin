...
func NewLesServer(eth *eth.AmazonCoin, config *eth.Config) (*LesServer, error) {
...
func newCostStats(db amzdb.Database) *requestCostStats {
...
func getChtRoot(db amzdb.Database, num uint64) common.Hash {
...
func storeChtRoot(db amzdb.Database, num uint64, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], num)
	db.Put(append(chtPrefix, encNumber[:]...), root[:])
}

func makeCht(db amzdb.Database) bool {
