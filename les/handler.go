...
func NewProtocolManager(chainConfig *params.ChainConfig, lightSync bool, networkId uint64, mux *event.TypeMux, engine consensus.Engine, peers *peerSet, blockchain BlockChain, txpool txPool, chainDb amzdb.Database, odr *LesOdr, txrelay *LesTxRelay, quitSync chan struct{}, wg *sync.WaitGroup) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Amazoncoin protocol")
	...
	log.Info("Light Amazoncoin protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Amazoncoin peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Amazoncoin message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Amazoncoin message arrived", "code", msg.Code, "bytes", msg.Size)
