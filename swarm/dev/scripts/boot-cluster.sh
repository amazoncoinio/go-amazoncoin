...
#
# The cluster contains a bootnode, a geta node and multiple swarm nodes, with
...
# static geta configuration
GETA_IP="192.168.33.3"
GETA_RPC_PORT="8545"
GETA_RPC_URL="http://${GETA_IP}:${GETA_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the geta and
# swarm nodes
start_bootnode() {
...
# start_geta_node starts a geta node with --datadir pointing at <base-dir>/geta
# and a single, unlocked account with password "geta"
start_geta_node() {
  local dir="${base_dir}/geta"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get geta account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GETA_IP}"
    --rpcport   "${GETA_RPC_PORT}"
    --verbosity "6"
  )

  start_node "geta" "${GETA_IP}" "$(which geta)" ${args[@]}
}
...
create_account() {
  ...
  geta --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
