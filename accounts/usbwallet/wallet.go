...
import (
	...
	amazoncoin "github.com/amazoncoinio/go-amazoncoin"
...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the AmazonCoin
...
type wallet struct {
	...
	deriveChain    amazoncoin.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Amazoncoin account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain amazoncoin.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
