...
import (
	...
	"github.com/amazoncoinio/go-amazoncoin/accounts/usbwallet/internal/trezor"
...
// or whether the Amazoncoin app was not started on it.
...
// Amazoncoin address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.AmazonCoinAddress)
	if _, err := w.trezorExchange(&trezor.AmazonCoinGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.AmazonCoinSignTx{
	...
	response := new(trezor.AmazonCoinTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.AmazonCoinTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
