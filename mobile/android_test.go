...
import (
	...
	"github.com/amazoncoinio/go-amazoncoin/internal/build"
)
...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.amazoncoin", new NodeConfig());
			node.start();
			...
			AmazonCoinClient ec = node.getAmazonCoinClient();
	...
	// signatures too. Regression test for go-amazoncoin issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogetaer
	...
	workspace, err := ioutil.TempDir("", "geta-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/amazoncoin/getatest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.amazoncoin", "github.com/amazoncoinio/go-amazoncoin/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "geta.aar"), "geta.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "amazoncoin", "getatest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.amazoncoin.getatest"
