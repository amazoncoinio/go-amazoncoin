...
import (
	...
	"github.com/amazoncoinio/go-amazoncoin/common/math"
...
// PublicAmazonCoinAPI provides an API to access Amazoncoin related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicAmazonCoinAPI struct {
	b Backend
}

// NewPublicAmazonCoinAPI creates a new Etheruem protocol API.
func NewPublicAmazonCoinAPI(b Backend) *PublicAmazonCoinAPI {
	return &PublicAmazonCoinAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicAmazonCoinAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current Amazoncoin protocol version this node supports
func (s *PublicAmazonCoinAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicAmazonCoinAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19AmazonCoin Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19AmazonCoin Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an Amazoncoin ECDSA signature for:
// keccack256("\x19AmazonCoin Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/amazoncoinio/go-amazoncoin/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19AmazonCoin Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/amazoncoinio/go-amazoncoin/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Amazoncoin signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Amazoncoin blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
...
//
// https://github.com/amazoncoinio/wiki/wiki/JSON-RPC#eth_sign
...
// Version returns the current Amazoncoin protocol version.
