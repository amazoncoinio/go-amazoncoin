...
type Config struct {
	Oracle common.Address // Amazoncoin address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the Amazoncoin service dependency to access the blockchain
	var apiBackend ethapi.Backend
	var amazoncoin *eth.AmazonCoin
	if err := ctx.Service(&amazoncoin); err == nil {
		apiBackend = amazoncoin.ApiBackend
	} else {
		var amazoncoin *les.LightAmazonCoin
		if err := ctx.Service(&amazoncoin); err == nil {
			apiBackend = amazoncoin.ApiBackend
		} else {
			return nil, err
...
func (r *ReleaseService) checkVersion() {
	...
	if version.Major > r.config.Major ||
		...
		howtofix := fmt.Sprintf("Please check https://github.com/amazoncoinio/go-amazoncoin/releases for new releases")
