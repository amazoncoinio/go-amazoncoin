...
import (
	...
	"github.com/amazoncoinio/go-amazoncoin/common/bitutil"
...
const (
	// bloomServiceThreads is the number of goroutines used globally by an AmazonCoin
...
func (eth *AmazonCoin) startBloomHandlers() {
...
// for the Amazoncoin header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size uint64 // section size to generate bloombits for

	db  amzdb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db amzdb.Database, size uint64) *core.ChainIndexer {
	backend := &BloomIndexer{
		db:   db,
		size: size,
	}
	table := amzdb.NewTable(db, string(core.BloomBitsIndexPrefix))
