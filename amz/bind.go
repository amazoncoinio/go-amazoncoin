...
// ContractBackend implements bind.ContractBackend with direct calls to AmazonCoin
// internals to support operating on contracts within subprotocols like eth and
// swarm.
//
// Internally this backend uses the already exposed API endpoints of the AmazonCoin
// object. These should be rewritten to internal Go method calls when the Go API
// is refactored to support a clean library use.
type ContractBackend struct {
	eapi  *ethapi.PublicAmazonCoinAPI        // Wrapper around the Amazoncoin object to access metadata
...
func NewContractBackend(apiBackend ethapi.Backend) *ContractBackend {
	return &ContractBackend{
		eapi:  ethapi.NewPublicAmazonCoinAPI(apiBackend),
...
// ContractCall implements bind.ContractCaller executing an Amazoncoin contract
// call with the specified data as the input. The pending flag requests execution
// against the pending block, not the stable head of the chain.
func (b *ContractBackend) CallContract(ctx context.Context, msg amazoncoin.CallMsg, blockNum *big.Int) ([]byte, error) {
...
// call with the specified data as the input. The pending flag requests execution
// against the pending block, not the stable head of the chain.
func (b *ContractBackend) PendingCallContract(ctx context.Context, msg amazoncoin.CallMsg) ([]byte, error) {
	out, err := b.bcapi.Call(ctx, toCallArgs(msg), rpc.PendingBlockNumber)
	return out, err
}

func toCallArgs(msg amazoncoin.CallMsg) ethapi.CallArgs {
...
func (b *ContractBackend) EstimateGas(ctx context.Context, msg amazoncoin.CallMsg) (*big.Int, error) {
